...
function fun_buscar_cedula() {
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url: PROTOCOL_HOST + '/modulos/usuarios/visor/modelo/buscar_cedula.php',
        type: 'POST',
        data: form_data,
        headers: {
        ...
        .done(function (response) {
          ...
          if (response.status === 'bien') {
            ...
            self.close();
            call_recuperar_session(fun_buscar_cedula);
          } else {
  ...
  $.confirm({
    ...
    buttons: {
      si: {
        text: 'Si',
        action: function () {
          fun_agregar_usuario(temp_cedula, temp_empresa);
        },
      },
      no: {
        text: 'No',
...
function fun_agregar_usuario(_cedula, _empresa) {
  let self = $.confirm({
    title: 'Error ',
    content: 'Cargando, espere...',
    typeAnimated: true,
    ...
    content: function () {
      return $.ajax({
        url: PROTOCOL_HOST + '/modulos/usuarios/visor/modelo/agregar.php',
        type: 'POST',
        data: {
          cedula: _cedula,
          empresa: _empresa,
        ...
        .done(function (response) {
          ...
          if (response.status === 'bien') {
            ...
            call_recuperar_session(function (k) {
              fun_agregar_usuario(_cedula, _empresa);
            });
          } else {
            self.setTitle(response.status);
