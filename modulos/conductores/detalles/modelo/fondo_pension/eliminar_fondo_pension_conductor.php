...
if (
    ...
    if (
        ...
        if (strcmp($database->status(), "bien") == 0) {
            ...
            $id_fondo_pension_conductor = htmlspecialchars($_POST['id_fondo_pension_conductor']);

            $mysql_query = "UPDATE fondo_pension_conductor ";
            $mysql_query .= "SET is_visible = 2 ";
            $mysql_query .= "WHERE id_fondo_pension_conductor = ? ; ";

            $mysql_stmt = mysqli_prepare($database->myconn, $mysql_query);
            $mysql_stmt->bind_param('i', $id_fondo_pension_conductor);

            if ($mysql_stmt->execute()) {
                $json_status = "bien";
                $json_message = "Certificado de Fondo de pensión ELIMINADO";
            } else {
                $json_status = "error";
