...
if (
    isset($_POST['form_12_placa_vehiculo_conductor']) &&
    isset($_POST['form_12_fecha_asignacion_vehiculo_conductor']) &&
    // isset($_POST['form_12_numero_vehiculo_conductor']) &&
    ...
    if (
        ...
        if (strcmp($database->status(), "bien") == 0) {
            ...
            $form_12_placa_vehiculo_conductor = htmlspecialchars(strtoupper($_POST["form_12_placa_vehiculo_conductor"]));
            $form_12_fecha_asignacion_vehiculo_conductor = getspecialdate(strtoupper($_POST["form_12_fecha_asignacion_vehiculo_conductor"]));
            // $form_12_numero_vehiculo_conductor = htmlspecialchars(strtoupper($_POST["form_12_numero_vehiculo_conductor"]));
            $id_conductor = htmlspecialchars(strtoupper(encrypt($_POST["id_conductor"], 2)));
            $form_id_usuario = $_SESSION['session_user'][1];
            $mysql_query = "call proc_conductor_vehiculo (?,?,?,?,@respuesta); ";

            $mysql_stmt = mysqli_prepare($database->myconn, $mysql_query);
            $mysql_stmt->bind_param(
                'isii',
                $form_12_placa_vehiculo_conductor,
                $form_12_fecha_asignacion_vehiculo_conductor,
                // $form_12_numero_vehiculo_conductor,
                $id_conductor,
                $form_id_usuario
            );
