...
if (
    ...
    if (
        ...
        if (strcmp($database->status(), "bien") == 0) {
            ...
            $id_vehiculo_conductor = htmlspecialchars($_POST['id_vehiculo_conductor']);

            $mysql_query = "UPDATE vehiculo_conductor ";
            $mysql_query .= "SET is_visible = 2 ";
            $mysql_query .= "WHERE id_vehiculo_conductor = ? ; ";

            $mysql_stmt = mysqli_prepare($database->myconn, $mysql_query);
            $mysql_stmt->bind_param('i', $id_vehiculo_conductor);

            if ($mysql_stmt->execute()) {
                $json_status = "bien";
                $json_message = "Asignacion de vehiculo ELIMINADA";
            } else {
                $json_status = "error";
