  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/modulos/conductores/visor/modelo/buscar_conductor.php',
        type: 'POST',
        data: {
          cedula: $temp_cedula,
  ...
  $.confirm({
    ...
    content: function () {
      ...
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/modulos/conductores/visor/modelo/agregar_por_cedula.php',
        type: 'POST',
        data: {
          cedula: _cedula,
          empresa: _empresa,
        ...
        .done(function (response) {
          ...
          if (response.status === 'bien') {
            ...
            response.status === 'csrf' ||
            response.status === ' pñukuk'
          ) {
            self.close();

            call_recuperar_session(function (k) {
              agregar_conductor(_cedula, _empresa);
            });
          } else {
            self.setTitle(response.status);
