...
class CreateMantenimientoSuper
{
    ...
    public function setMantenimiento(
        $_id_vehiculo = 1,
        $_orden_servicio = 1,
        $_nro_orden_servicio = 'No',
        $_firma = '/images/sin_firma.png',
        $_id_usuario = 1
    ) {
        $mysqlQuery = "CALL proc_crear_mantenimiento (?,?,?,?,?,@respuesta); ";

        $mysqlStmt = mysqli_prepare($this->databaseConnection, $mysqlQuery);
        $mysqlStmt->bind_param('iissi', $_id_vehiculo, $_orden_servicio, $_nro_orden_servicio, $_firma, $_id_usuario);
