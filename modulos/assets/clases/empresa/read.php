...
class ReadEmpresa
{
    ...
    public function getEmpresaInformacion(
        ...
        $mysqlQuery .= "empr.id_estado,esta.nombre_estado_empresa, ";
        ...
        #JOIN
        $mysqlQuery .= "LEFT JOIN estado_empresa esta ON empr.id_estado = esta.id_estado_empresa ";
                ...
                while ($row = $mysqlResult->fetch_assoc()) {
                    array_push($mysqlArrayEmpresa,
                        array(
                            ...
                            "estado" => htmlspecialchars($row['nombre_estado_empresa']),
    ...
    public function getEmpresaBuscador(
        ...
        $mysqlQueryColumns .= "ciud.nombre_ciudad,esta.nombre_estado_empresa ";
        #Tabla
        $mysqlQuery = "FROM ";
        $mysqlQuery .= "empresa empr ";
        $mysqlQuery .= "LEFT JOIN ciudad ciud ON empr.id_ciudad = ciud.id_ciudad ";
        ...
        $mysqlQuery .= "AND empr.id_estado LIKE ? ";
        #Ordenamiento
        ...
        $mysqlStmt->bind_param('ss', $_condicional_['CONTENT'],$_condicional_['ESTADO']);
        # Se ejecuta
        if ($mysqlStmt->execute()) {
            # Obtener
            $mysqlResult = $mysqlStmt->get_result();
                    ...
                    while ($row = $mysqlResult->fetch_assoc()) {
                        array_push($mysqlArrayEmpresa,
                            array(
                                ...
                                "ciudad" => htmlspecialchars($row['nombre_ciudad']),
                                "estado" => htmlspecialchars($row['nombre_estado_empresa']),
                                "opciones" => encrypt($row['id_empresa'], 1),
                            )
                        );
