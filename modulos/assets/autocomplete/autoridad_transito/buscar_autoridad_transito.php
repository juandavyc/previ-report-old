...
if (isset($_POST["palabra"]) &&
    ...
    if (isset($headers['csrf-token']) && hash_equals($headers['csrf-token'], $_SESSION['csrf_token'])) {
        ...
        if (strcmp($database->status(), "bien") == 0) {
            ...
            $mysql_query = "SELECT ";
            $mysql_query .= "id_autoridad_de_transito, nombre_autoridad_de_transito ";
            $mysql_query .= "FROM ";
            $mysql_query .= "autoridad_de_transito ";
            $mysql_query .= "WHERE ";
            $mysql_query .= "nombre_autoridad_de_transito LIKE ? LIMIT 5;";
                ...
                if (mysqli_num_rows($mysql_result) > 0) {
                    ...
                    while ($fila = $mysql_result->fetch_assoc()) {
                        $json_options[$php_desde_ini] = array(
                            "id" => htmlspecialchars($fila['id_autoridad_de_transito']),
                            "nombre" => htmlspecialchars($fila['nombre_autoridad_de_transito']),
                        );
