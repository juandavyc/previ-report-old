...
export function fun_info_empresa(_id_elemento) {
  let self = $.confirm({
    title: false,
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/modulos/empresas/visor/modelo/buscador_informacion.php',
        type: 'POST',
        data: {
        ...
        .done(function (response) {
          if (response.status === 'bien') {
            self.setTitle('Información de la Empresa');
...
export function fun_link_empresa(_id_empresa) {
  $.confirm({
    title: 'Alerta',
...
function fun_asignar_datos(_response) {
  ...
  <i class="fas fa-briefcase fa-3x"></i></div><div class="col-12"><hr class="hr-text" data-content="Información">
