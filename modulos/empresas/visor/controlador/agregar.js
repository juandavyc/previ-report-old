...
let fun_info_empresa = null;
let fun_link_empresa = null;

export function fun_agregar_functions(_function_info, _function_link) {
  fun_info_empresa = _function_info;
  fun_link_empresa = _function_link;
}
...
$('#agregar_resultados_body').on('click', '#btn_info_empresa', function (e) {
  fun_info_empresa($(this).attr('btn-id'));
  e.preventDefault();
  return false;
});

$('#agregar_resultados_body').on('click', '#btn_link_empresa', function (e) {
  fun_link_empresa($(this).attr('btn-id'));
  e.preventDefault();
  return false;
});
...
function fun_buscar_nit() {
  ...
  $.confirm({
    ...
    content: function () {
      var self = this;
      return $.ajax({
        url: PROTOCOL_HOST + '/modulos/empresas/visor/modelo/buscar_nit.php',
        type: 'POST',
        data: form_data,
        headers: {
          ...
          if (response.status === 'bien') {
            ...
            $('#agregar_resultados_body').html(
              ...
              <ul>
                ...
                <li>Si desea <b>crear una empresa con este NIT</b>, pulse en <b>(Crear Empresa).</li>
              </ul>  
              <center>
              <button class="primary small icon solid fa-arrow-right" id="btn_crear_empresa"> Crear Empresa</button>        
              </center>`
            );
...
$('#agregar_resultados_body').on('click', '#btn_crear_empresa', function (e) {
  $.confirm({
    title: 'Alerta',
    content:
      '<center>' +
      'Esta por crear una empresa este NIT  <br>' +
      '<b> ( ' +
...
function fun_agregar_empresa(_nit) {
  let self = $.confirm({
    title: 'Error ',
    content: 'Cargando, espere...',
    typeAnimated: true,
    ...
    content: function () {
      return $.ajax({
        url: PROTOCOL_HOST + '/modulos/empresas/visor/modelo/agregar.php',
        type: 'POST',
        data: {
          nit: _nit,
        },
