...
$('#buscador_resultados_body').on('click', '#btn_info_empresa', function (e) {
  fun_info_empresa($(this).attr('btn-id'));
  e.preventDefault();
  return false;
});

$('#buscador_resultados_body').on('click', '#btn_link_empresa', function (e) {
...
function fun_buscador(_type) {
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url: PROTOCOL_HOST + '/modulos/empresas/visor/modelo/buscador.php',
        type: 'POST',
        data: form_data,
        headers: {
