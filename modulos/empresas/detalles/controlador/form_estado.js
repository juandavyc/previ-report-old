...
$('#form_2_estado_empresa').on('submit', function (e) {
  $.confirm({
    title: 'Alerta!',
    content:
...
function fun_form_2_submit() {
  let formdata = new FormData($('#form_2_estado_empresa')[0]);
  formdata.append('form_2_id_empresa', const_id);
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST + '/modulos/empresas/detalles/modelo/estado/estado.php',
        type: 'POST',
        data: formdata,
        contentType: false,
        cache: false,
