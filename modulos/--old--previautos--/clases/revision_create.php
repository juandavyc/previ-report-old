...
class CreateRevision
{
    ...
    public function setDatos(
        ...
        $_vencimiento = '2000-01-01',
        $_src = '/images/sin_imagen.png',
        $_id_vehiculo = 1,
        $_id_usuario = 1,
    ) {


        $mysqlQuery = "CALL proc_previautos_revision (?,?,?,?,?,?,?,@respuesta); ";

        $mysqlStmt = mysqli_prepare($this->databaseConnection, $mysqlQuery);
        $mysqlStmt->bind_param('issssii', $_tipo, $_bimensual, $_expedicion, $_vencimiento, $_src, $_id_vehiculo, $_id_usuario);
        ...
        if ($mysqlStmt->execute()) {
            ...
            if ($mysqlStmt->execute()) {
                ...
                $this->arrayResponse = array(
                    ...
                    'placa' => isset($mysqlDecode[3]) ? $mysqlDecode[3] : '',
                );
            } else {
                $this->arrayResponse = array(
                    'status' => 'error',
