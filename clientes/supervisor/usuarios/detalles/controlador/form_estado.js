...
export function fun_estado_accordion(_funtion_name) {
  accordion_busqueda = _funtion_name;
}

$('#form_1_estado_usuario').on('submit', function (e) {
  $.confirm({
    title: 'Alerta!',
    content:
...
function fun_form_1_submit() {
  ...
  let formdata = new FormData($('#form_1_estado_usuario')[0]);
  formdata.append('form_1_id_usuario', const_id);
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/clientes/supervisor/usuarios/detalles/modelo/estado_usuario/estado.php',
        type: 'POST',
        data: formdata,
        contentType: false,
        cache: false,
