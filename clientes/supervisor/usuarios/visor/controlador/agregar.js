...
function fun_buscar_cedula() {
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url: PROTOCOL_HOST + '/clientes/supervisor/usuarios/visor/modelo/buscar_cedula.php',
        type: 'POST',
        data: form_data,
        headers: {
...
function fun_agregar_usuario(_cedula) {
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url: PROTOCOL_HOST + '/clientes/supervisor/usuarios/visor/modelo/agregar.php',
        type: 'POST',
        data: {
          cedula: _cedula,
        },
        ...
        .done(function (response) {
          ...
          if (response.status === 'bien') {
            ...
            window.location.replace(
              PROTOCOL_HOST +
                '/clientes/supervisor/usuarios/detalles/index.php?id=' +
                response.message
            );
          } else if (
            response.status === 'csrf' ||
