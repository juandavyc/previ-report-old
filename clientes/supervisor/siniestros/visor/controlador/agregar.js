...
//   url_select_ajax:
//     PROTOCOL_HOST +
//     '/clientes/supervisor/assets/autocomplete/vehiculo_conductor/buscar_empresa.php',
//   url_insert_ajax:
//     PROTOCOL_HOST + '/clientes/supervisor/siniestros/visor/modelo/crear_vehiculo.php',
//   input_value_default: '',
//   input_select_default: '0',
...
autocomplete_with_insert_child({
  ...
  url_select_ajax:
    PROTOCOL_HOST +
    '/clientes/supervisor/assets/autocomplete/conductor_general/buscar_empresa.php',
  url_insert_ajax:
    PROTOCOL_HOST + '/clientes/supervisor/siniestros/visor/modelo/crear_conductor.php',
  input_value_default: '',
  input_select_default: '0',
  input_father_name: 'EMPRESA',
  input_father_text: 'form_1_empresa_input',
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/clientes/supervisor/siniestros/visor/modelo/buscar_vehiculo_siniestro.php',
        type: 'POST',
        data: {
          placa: temp_placa,
        },
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/clientes/supervisor/siniestros/visor/modelo/buscar_datos_conductor.php',
        type: 'POST',
        data: form_data,
        headers: {
          'csrf-token': $('meta[name="csrf-token"]').attr('content'),
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/clientes/supervisor/siniestros/visor/modelo/crear_siniestro.php',
        type: 'POST',
        data: datos,
        headers: {
          'csrf-token': $('meta[name="csrf-token"]').attr('content'),
        ...
        .done(function (response) {
          ...
          if (response.status === 'bien') {
            ...
            window.location.replace(
              PROTOCOL_HOST +
                '/clientes/supervisor/siniestros/detalles/index.php?id=' +
                response.message
            );
          } else if (response.status === 'sin_resultados') {
            self.setTitle('Completado!');
