...
require DOCUMENT_ROOT . '/clientes/supervisor/assets/clases/usuario/create.php';
    ...
    if ($json_host['status'] == "ok") {
        ...
        if (
            ...
            if (strcmp($database->status(), "bien") == 0) {
                ...
                $arrayResponse = $usuario->setUsuarioCedula($cedula, $id_usuario);

                if ($arrayResponse['status'] == 'bien') {
                    $json_status = $arrayResponse['status'];
                    $json_message = $arrayResponse['id'];
                } else {
                    $json_status = $arrayResponse['status'];
