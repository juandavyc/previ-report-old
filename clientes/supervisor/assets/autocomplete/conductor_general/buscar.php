...
if (isset($_POST["palabra"]) &&
    ...
    if (isset($headers['csrf-token']) && hash_equals($headers['csrf-token'], $_SESSION['csrf_token'])) {
        ...
        if (strcmp($database->status(), "bien") == 0) {
            ...
            $mysql_query = "SELECT ";
            $mysql_query .= "id_conductor,numero_documento ";
            $mysql_query .= "FROM ";
            $mysql_query .= "conductor ";
            $mysql_query .= "WHERE ";
            $mysql_query .= "numero_documento LIKE ? AND id_empresa LIKE ? LIMIT 5;";
            ...
            $mysql_stmt->bind_param('ss', $form_palabra,$empresa);
                ...
                if (mysqli_num_rows($mysql_result) > 0) {
                    ...
                    while ($fila = $mysql_result->fetch_assoc()) {
                        $json_options[$php_desde_ini] = array(
                            "id" => htmlspecialchars($fila['id_conductor']),
                            "nombre" => htmlspecialchars($fila['numero_documento']),
                        );
