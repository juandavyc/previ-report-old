...
if (isset($_POST["palabra"]) &&
    ...
    include DOCUMENT_ROOT . '/clientes/supervisor/
    ...
    if (isset($headers['csrf-token']) && hash_equals($headers['csrf-token'], $_SESSION['csrf_token'])) {
        ...
        if (strcmp($database->status(), "bien") == 0) {
            ...
            $mysql_query = "SELECT ";
            $mysql_query .= "id_linea,nombre_linea ";
            $mysql_query .= "FROM ";
            $mysql_query .= "linea ";
            $mysql_query .= "WHERE ";
            $mysql_query .= "nombre_linea LIKE ? AND ";
            $mysql_query .= "id_marca = ? ";
            $mysql_query .= "LIMIT 5;";
                ...
                if (mysqli_num_rows($mysql_result) > 0) {
                    ...
                    while ($fila = $mysql_result->fetch_assoc()) {
                        $json_options[$php_desde_ini] = array(
                            "id" => htmlspecialchars($fila['id_linea']),
                            "nombre" => htmlspecialchars($fila['nombre_linea']),
                        );
