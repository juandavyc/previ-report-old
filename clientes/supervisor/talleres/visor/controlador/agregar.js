...
autocomplete_with_insert_father({
  ...
  url_insert_ajax:
    PROTOCOL_HOST + 'clientes/supervisor/talleres/visor/modelo/crear_empresas.php',
  input_value_default: '',
...
// console.log($temp_empresa);
  ...
  $.confirm({
    ...
    content: function () {
      var self = this;
      return $.ajax({
        url: PROTOCOL_HOST + '/clientes/supervisor/talleres/visor/modelo/buscar_nit.php',
        type: 'POST',
        data: form_data,
        headers: {
...
function fun_agregar_taller(_nit,_empresa) {
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url: PROTOCOL_HOST + '/clientes/supervisor/talleres/visor/modelo/agregar.php',
        type: 'POST',
        data: {
          nit: _nit,
          empresa: _empresa,
        ...
        .done(function (response) {
          ...
          if (response.status === 'bien') {
            ...
            window.location.replace(
              PROTOCOL_HOST +
                '/clientes/supervisor/talleres/detalles/index.php?id=' +
                response.message
            );
          } else if (
            response.status === 'csrf' ||
