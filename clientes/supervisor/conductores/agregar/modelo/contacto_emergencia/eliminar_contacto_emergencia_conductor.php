    ...
    if ($json_host['status'] == "ok") {
        ...
        if (
            ...
            if (strcmp($database->status(), "bien") == 0) {
                ...
                $id_contacto_emergencia_conductor = encrypt($_POST['id_contacto_emergencia_conductor'], 2);

                $mysql_query = "UPDATE contacto_de_emergencia_conductor ";
                $mysql_query .= "SET is_visible = 2 ";
                $mysql_query .= "WHERE id_contacto_de_emergencia_conductor = ? ; ";

                $mysql_stmt = mysqli_prepare($database->myconn, $mysql_query);
                $mysql_stmt->bind_param('i', $id_contacto_emergencia_conductor);

                if ($mysql_stmt->execute()) {
                    $json_status = "bien";
                    $json_message = "Contacto de emergencia ELIMINADO";
                } else {
                    $json_status = "error";
