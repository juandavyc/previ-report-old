...
if (
    isset($_POST['_id_comparendo_conductor']) &&
    ...
    if ($json_host['status'] == "ok") {
        ...
        if (
            ...
            if (strcmp($database->status(), "bien") == 0) {
                ...
                $_id_comparendo_conductor = htmlspecialchars($_POST['_id_comparendo_conductor']);

                $mysql_query = "UPDATE comparendo_conductor ";
                $mysql_query .= "SET is_visible = 2 ";
                $mysql_query .= "WHERE id_comparendo_conductor = ? ; ";

                $mysql_stmt = mysqli_prepare($database->myconn, $mysql_query);
                $mysql_stmt->bind_param('i', $_id_comparendo_conductor);

                if ($mysql_stmt->execute()) {
                    $json_status = "bien";
                    $json_message = "Comparendo ELIMINADO";
                } else {
                    $json_status = "error";
