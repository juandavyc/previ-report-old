...
class CreateUsuario
{
    ...
    public function setUsuarioCedula($_cedula = 1, $_empresa = 1, $_id_usuario = 1)
    {
        $mysqlQuery = "CALL proc_crear_cedula_usuario (?,?,?,@respuesta); ";

        $mysqlStmt = mysqli_prepare($this->databaseConnection, $mysqlQuery);
        $mysqlStmt->bind_param('sii', $_cedula, $_empresa, $_id_usuario);
        ...
        if ($mysqlStmt->execute()) {
            ...
            if ($mysqlStmt->execute()) {
                ...
                $this->arrayResponse = array(
                    ...
                    'cedula' => isset($mysqlDecode[3]) ? $mysqlDecode[3] : '',
                );
            } else {
                $this->arrayResponse = array(
                    'status' => 'error',
