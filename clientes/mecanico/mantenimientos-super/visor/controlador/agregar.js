...
autocomplete_with_insert_father({
  ...
  url_select_ajax:
    PROTOCOL_HOST +
    '/clientes/mecanico/assets/autocomplete/vehiculo_conductor/buscar.php',
  url_insert_ajax:
    PROTOCOL_HOST + '/clientes/mecanico/siniestros/visor/modelo/crear_vehiculo.php',
  input_value_default: '',
...
$('#form_1_nuevo_mantenimiento').on('submit', function (e) {
  $.confirm({
    ...
    content:
      '<center>' +
      'Esta por Asignar un mantenimiento para: <br>' +
      '<b> ( ' +
...
$('input[type=radio][name=form_1_orden_servicio]').click(function () {
  if ($(this).val() == '1') {
    $('#form_1_numero_orden_servicio').attr('required', true);
  } else {
    $('#form_1_numero_orden_servicio').attr('required', false);
  }
});
    ...
    let self = $.confirm({
      ...
      content: function () {
        return $.ajax({
          url:
            PROTOCOL_HOST +
            '/clientes/mecanico/mantenimientos-super/visor/modelo/crear_mantenimiento.php',
          type: 'POST',
          data: form_data,
          headers: {
          ...
          .done(function (response) {
            ...
            if (response.status === 'bien') {
              ...
              window.location.replace(
                PROTOCOL_HOST +
                  '/clientes/mecanico/mantenimientos-taller/agregar/index.php?id=' +
                  response.message
              );
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/clientes/mecanico/mantenimientos-super/visor/modelo/informacion_vehiculo.php',
        type: 'POST',
        data: {
