...
export function fun_licencia_transito_accordion(_funtion_name) {
  accordion_busqueda = _funtion_name;
}

$('.licencia-images-link').viewbox({
  margin: 50,
  resizeDuration: 300,
  openDuration: 200,
  closeDuration: 200,
...
$('#form_12_licencia_transito').on('submit', function (e) {
...
function fun_form_12() {
  let formdata = new FormData($('#form_12_licencia_transito')[0]);
  formdata.append('form_12_id_vehiculo', const_id_vehiculo);
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/clientes/administrador/vehiculos/agregar/modelo/licencia_transito/form.php',
        type: 'POST',
        data: formdata,
        contentType: false,
        cache: false,
