    ...
    if ($json_host['status'] == "ok") {
        ...
        if (
            ...
            if (strcmp($database->status(), "bien") == 0) {
                ...
                $mysql_query = "SELECT ";
                $mysql_query .= "cert.numero_certificado, cert.fecha_expedicion_certificado, cert.fecha_vencimiento_certificado, ";
                $mysql_query .= "cert.foto_certificado, usu.nombre_usuario, usu.apellido_usuario, ";
                $mysql_query .= "enti.nombre_entidad_certificado, veh.placa_vehiculo, cert.fecha_formulario ";
                $mysql_query .= "FROM certificado cert ";
                $mysql_query .= "INNER JOIN usuario usu ON usu.id_usuario = cert.id_usuario ";
                $mysql_query .= "INNER JOIN entidad_certificado enti ON enti.id_entidad_certificado = cert.id_entidad_certificado ";
                $mysql_query .= "INNER JOIN vehiculo veh ON veh.id_vehiculo = cert.id_vehiculo ";
                $mysql_query .= "WHERE ";
                $mysql_query .= "cert.id_certificado = ? ";
                $mysql_query .= "ORDER BY cert.id_certificado DESC LIMIT 0,1 ; ";
