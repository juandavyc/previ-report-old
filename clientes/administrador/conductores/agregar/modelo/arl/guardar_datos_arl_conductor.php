...
if (
    isset($_POST['form_4_foto_arl']) &&
    isset($_POST['form_4_arl_conductor']) &&
    isset($_POST['form_4_arl_fecha_afiliacion_conductor']) &&
    isset($_POST['form_4_arl_estado_conductor']) &&
    ...
    if ($json_host['status'] == "ok") {
        ...
        if (
            ...
            if (strcmp($database->status(), "bien") == 0) {
                ...
                $form_4_foto_arl = htmlspecialchars(($_POST["form_4_foto_arl"]));
                $form_4_arl_conductor = htmlspecialchars(strtoupper($_POST["form_4_arl_conductor"]));
                $form_4_fecha_afiliacion_arl = getspecialdate(strtoupper($_POST["form_4_arl_fecha_afiliacion_conductor"]));
                $form_4_estado_arl_conductor = htmlspecialchars(strtoupper($_POST["form_4_arl_estado_conductor"]));
                $form_4_id_conductor = htmlspecialchars(strtoupper(encrypt($_POST["id_conductor"],2)));
                $form_id_usuario = $_SESSION['session_user'][1];
                $mysql_query = "call proc_conductor_arl (?,?,?,?,?,?,@respuesta); ";

// var_dump($form_4_id_conductor);

                $mysql_stmt = mysqli_prepare($database->myconn, $mysql_query);
                $mysql_stmt->bind_param(
                    'sisiii',
                    $form_4_foto_arl,
                    $form_4_arl_conductor,
                    $form_4_fecha_afiliacion_arl,
                    $form_4_estado_arl_conductor,
                    $form_4_id_conductor,
                    $form_id_usuario
