    ...
    if ($json_host['status'] == "ok") {
        ...
        if (
            ...
            if (strcmp($database->status(), "bien") == 0) {
                ...
                $id_arl_conductor = encrypt($_POST['id_arl_conductor'],2);

                $mysql_query = "UPDATE arl_conductor ";
                $mysql_query .= "SET is_visible = 2 ";
                $mysql_query .= "WHERE id_arl_conductor = ? ; ";

                $mysql_stmt = mysqli_prepare($database->myconn, $mysql_query);
                $mysql_stmt->bind_param('i', $id_arl_conductor);

                if ($mysql_stmt->execute()) {
                    $json_status = "bien";
                    $json_message = "Certificado de arl ELIMINADO";
                } else {
                    $json_status = "error";
