...
require DOCUMENT_ROOT . '/clientes/administrador/assets/clases/preoperacional/supervisor/read.php';
    ...
    if ($json_host['status'] == "ok") {
        ...
        if (
            ...
            if (strcmp($database->status(), "bien") == 0) {
                ...
                $id_preoperacional = htmlspecialchars($_POST['id_elemento']);

                $preoperacional = new ReadPreoperacionalSuper($database->myconn);

                $arrayResponse = $preoperacional->getPreoperacionalSuper(
                    array(
                        'TYPE' => 'ID',
                        'VALUE' => encrypt($id_preoperacional, 2),
                        'LIMITE' => '0,1',
                    )
                );
