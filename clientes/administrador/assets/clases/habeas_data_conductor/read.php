...
class HabeasDataConductor
{
    ...
    public function getHabeasDataConductor(
        ...
        $mysql_query = "SELECT ";
        $mysql_query .= "veh.id_vehiculo, CONCAT (usu.nombre_usuario,'  ', usu.apellido_usuario) as nombre_completo_habeas,
        habcon.firma_habeas, dat.nombre_habeas_data, habcon.fecha_formulario_habeas_conductor,con.id_conductor  ";
        
        $mysql_query .= "FROM conductor con ";

        $mysql_query .= "LEFT JOIN usuario usu ON con.id_usuario = usu.id_usuario ";
        $mysql_query .= "LEFT JOIN habeas_conductor habcon ON usu.id_usuario = habcon.id_usuario ";
