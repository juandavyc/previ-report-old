...
if (isset($_POST["palabra"]) &&
    ...
    if (isset($headers['csrf-token']) && hash_equals($headers['csrf-token'], $_SESSION['csrf_token'])) {
        ...
        if (strcmp($database->status(), "bien") == 0) {
            ...
            $mysql_query = "SELECT ";
            $mysql_query .= "id_entidad_certificado,nombre_entidad_certificado ";
            $mysql_query .= "FROM ";
            $mysql_query .= "entidad_certificado ";
            $mysql_query .= "WHERE ";
            $mysql_query .= "nombre_entidad_certificado LIKE ? LIMIT 5;";
                ...
                if (mysqli_num_rows($mysql_result) > 0) {
                    ...
                    while ($fila = $mysql_result->fetch_assoc()) {
                        $json_options[$php_desde_ini] = array(
                            "id" => htmlspecialchars($fila['id_entidad_certificado']),
                            "nombre" => htmlspecialchars($fila['nombre_entidad_certificado']),
                        );
